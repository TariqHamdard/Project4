...
public class GentlyDownTheStream {
    ...
    public List<String> sortedFruitsException() {
        return fruits.stream().filter(x -> !x.startsWith("A")).sorted().collect(Collectors.toList());
    }


    // TODO - return a list with the first 2 elements of a sorted list of fruits
    public List<String> sortedFruitsFirstTwo() {
        return fruits.stream().sorted().collect(Collectors.toList()).subList(0, 2);
    }
...
return veggies.stream().sorted(Comparator.reverseOrder()).collect(Collectors.toList());
    ...
    public List<Integer> topTenUnique() {
        return integerValues.stream().sorted(Comparator.reverseOrder()).distinct().limit(10).collect(Collectors.toList());
    }
    ...
    public List<Integer> topTenUniqueOdd() {
        return integerValues.stream().sorted(Comparator.reverseOrder()).distinct().filter(x -> x % 2 != 0).limit(10).collect(Collectors.toList());
    }

    // TODO - return a single value that represents the average of all of your random numbers
    public Double average() {
        return integerValues.stream().mapToInt(i -> i).average().getAsDouble();
    }
}
